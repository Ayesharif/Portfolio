            ...
            <div className="flex items-center gap-3">
              {cert.image ? (
                <img
                  src={cert.image}
                  alt={cert.title}
                  className="w-12 h-12 rounded-xl object-cover border border-amber-500/30 shrink-0 bg-slate-950"
                  onError={(e) => {
                    e.currentTarget.style.display = 'none';
                  }}
                />
              ) : (
                <div className="w-12 h-12 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400 font-mono text-xs font-bold shrink-0">
                  CERT
                </div>
              )}
              <div>
