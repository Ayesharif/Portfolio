...
export default function AdminPanel() {
  ...
  const saveModalItem = async (e) => {
    e.preventDefault();
    const { type, isNew, data: itemData } = editingItem;

    try {
      if (type === "experience") {
        if (isNew) await addExperience(itemData);
        else await updateExperience(itemData.id, itemData);
      } else if (type === "project") {
        if (isNew) await addProject(itemData);
        else await updateProject(itemData.id, itemData);
      } else if (type === "skill") {
        if (isNew) await addSkill(itemData);
        else await updateSkill(itemData.id, itemData);
      } else if (type === "certification") {
        if (isNew) await addCertification(itemData);
        else await updateCertification(itemData.id, itemData);
      } else if (type === "education") {
        if (isNew) await addEducation(itemData);
        else await updateEducation(itemData.id, itemData);
      }
